 ...
 * <h2>Logging:</h2>
 *
 * <p>All job output (inputs, AI responses, errors) is logged through {@code OrthJobHelper.log()}
 * and persisted to executor log files. Logs are accessible through the admin console for each
 * execution.
 *
 * <h2>Error Handling:</h2>
 *
 * <p>Failures are reported via {@code OrthJobHelper.handleFail()} and marked as FAIL status in
...
public class AIOrthJob {
    ...
    public void ollamaJobHandler() {
        ...
        OrthJobHelper.log("<br><br><b>【Input】: " + ollamaParam.getInput() + "</b><br><br>");

        String response = executeOllamaInference(ollamaParam);

        OrthJobHelper.log("<br><br><b>【Output】: " + response + "</b><br><br>");
    }
    ...
    private OllamaParam parseOllamaParam(String param) {
        try {
            ...
            OrthJobHelper.log(new RuntimeException("OllamaParam parse error", e));
            OrthJobHelper.handleFail();
            return null;
        }
    }
    ...
    private boolean validateOllamaParam(OllamaParam ollamaParam) {
        if (ollamaParam.getInput() == null || ollamaParam.getInput().isBlank()) {
            OrthJobHelper.log("input is empty.");
            OrthJobHelper.handleFail();
            return false;
        }
        return true;
     ...
     */
    @OrthJob("difyWorkflowJobHandler")
    public void difyWorkflowJobHandler() throws Exception {
        ...
        OrthJobHelper.log("<br><br><b>【inputs】: " + difyParam.getInputs() + "</b><br><br>");
    ...
    private DifyParam parseDifyParam(String param) {
        try {
            ...
            OrthJobHelper.log(new RuntimeException("DifyParam parse error", e));
            OrthJobHelper.handleFail();
            return null;
        }
    }
    ...
    private boolean validateDifyParam(DifyParam difyParam) {
        if (difyParam.getBaseUrl() == null || difyParam.getApiKey() == null) {
            OrthJobHelper.log("baseUrl or apiKey invalid.");
            OrthJobHelper.handleFail();
            return false;
        }
        return true;
