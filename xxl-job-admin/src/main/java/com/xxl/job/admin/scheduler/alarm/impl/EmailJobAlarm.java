 ...
 *
 * <ul>
 *   <li>Configured in {@link XxlJobInfo#getAlarmEmail()}
...
public class EmailJobAlarm implements JobAlarm {
    ...
    private boolean hasAlarmEmail(XxlJobInfo info) {
        return info != null
    ...
    private String buildAlarmContent(XxlJobLog jobLog) {
        ...
        if (jobLog.getHandleCode() > 0
                && jobLog.getHandleCode() != XxlJobContext.HANDLE_CODE_SUCCESS) {
    ...
    private XxlJobGroup loadJobGroup(int jobGroupId) {
        return XxlJobAdminBootstrap.getInstance().getXxlJobGroupMapper().load(jobGroupId);
    }

    /** Formats the email body using the HTML template. */
    private String formatEmailBody(XxlJobGroup group, XxlJobInfo info, String alarmContent) {
    ...
    private boolean sendToAllRecipients(
        ...
        var bootstrap = XxlJobAdminBootstrap.getInstance();
    ...
    private void sendEmail(
            XxlJobAdminBootstrap bootstrap,
            String senderName,
            String recipient,
            String subject,
            String body)
