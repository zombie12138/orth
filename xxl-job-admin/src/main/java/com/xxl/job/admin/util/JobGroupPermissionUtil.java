...
public class JobGroupPermissionUtil {
    ...
    public static boolean hasJobGroupPermission(LoginInfo loginInfo, int jobGroup) {
        if (XxlSsoHelper.hasRole(loginInfo, Consts.ADMIN_ROLE).isSuccess()) {
            ...
            List<String> jobGroups =
                    (loginInfo.getExtraInfo() != null
                                    && loginInfo.getExtraInfo().containsKey("jobGroups"))
                            ? StringTool.split(loginInfo.getExtraInfo().get("jobGroups"), ",")
                            : new ArrayList<>();
    ...
    public static LoginInfo validJobGroupPermission(HttpServletRequest request, int jobGroup) {
        ...
        if (!(loginInfoResponse.isSuccess()
                && hasJobGroupPermission(loginInfoResponse.getData(), jobGroup))) {
            throw new RuntimeException(
                    I18nUtil.getString("system_permission_limit")
                            + "[username="
                            + loginInfoResponse.getData().getUserName()
                            + "]");
        }
    ...
    public static List<XxlJobGroup> filterJobGroupByPermission(
        ...
        if (XxlSsoHelper.hasRole(loginInfoResponse.getData(), Consts.ADMIN_ROLE).isSuccess()) {
            ...
            List<String> jobGroups =
                    (loginInfoResponse.getData().getExtraInfo() != null
                                    && loginInfoResponse.getData().getExtraInfo().get("jobGroups")
                                            != null)
                            ? StringTool.split(
                                    loginInfoResponse.getData().getExtraInfo().get("jobGroups"),
                                    ",")
