...
public class XxlJobServiceImpl implements XxlJobService {
    ...
    public Response<String> add(XxlJobInfo jobInfo, LoginInfo loginInfo) {
        ...
        if (GlueTypeEnum.BEAN == GlueTypeEnum.match(jobInfo.getGlueType())
                && StringTool.isBlank(jobInfo.getExecutorHandler())) {
            ...
            for (String childJobIdItem : childJobIds) {
                if (StringTool.isNotBlank(childJobIdItem) && StringTool.isNumeric(childJobIdItem)) {
                    XxlJobInfo childJobInfo =
                            xxlJobInfoMapper.loadById(Integer.parseInt(childJobIdItem));
    ...
    public Response<String> update(XxlJobInfo jobInfo, LoginInfo loginInfo) {
        ...
        boolean scheduleDataNotChanged =
                jobInfo.getScheduleType().equals(exists_jobInfo.getScheduleType())
                        && jobInfo.getScheduleConf()
                                .equals(exists_jobInfo.getScheduleConf()); // 触发配置如果不变，避免重复计算；
        if (exists_jobInfo.getTriggerStatus() == TriggerStatus.RUNNING.getValue()
        ...
        logger.info(
                ...
                loginInfo.getUserName(),
                "jobinfo-update",
                GsonTool.toJson(exists_jobInfo));
