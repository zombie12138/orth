...
/**
 * Integration tests for {@link JobRegistryMapper}.
 *
...
public class JobRegistryMapperTest {
    private static final Logger logger = LoggerFactory.getLogger(JobRegistryMapperTest.class);
    ...
    public void testExecutorRegistryOperations() {
        ...
        int upsertResult =
                jobRegistryMapper.registrySaveOrUpdate(
        ...
        List<JobRegistry> activeRegistries =
                jobRegistryMapper.findAll(TEST_TIMEOUT_SECONDS, timeoutThreshold);
        assertNotNull(activeRegistries, "Active registries should not be null");

        // Remove dead executor entries
        int removeResult = jobRegistryMapper.removeDead(Arrays.asList(TEST_DEAD_REGISTRY_ID));
