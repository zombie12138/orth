...
import com.abyss.orth.admin.scheduler.exception.OrthJobException;
...
public class FixRateScheduleType extends ScheduleType {
     ...
     *
     * <p>The interval is stored in {@link JobInfo#getScheduleConf()} as a string representation of
     * seconds. This method parses the interval and adds it to the reference time.
     *
     ...
     * @throws OrthJobException if the schedule configuration is not a valid positive integer
     */
    @Override
    public Date generateNextTriggerTime(JobInfo jobInfo, Date fromTime) {
        try {
            long intervalSeconds = Long.parseLong(jobInfo.getScheduleConf());

            if (intervalSeconds <= 0) {
                throw new OrthJobException(
