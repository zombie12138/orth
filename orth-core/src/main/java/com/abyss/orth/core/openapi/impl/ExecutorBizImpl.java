...
import com.abyss.orth.core.handler.impl.GlueJobHandler;
import com.abyss.orth.core.handler.impl.ScriptJobHandler;
...
public class ExecutorBizImpl implements ExecutorBiz {
    ...
    public Response<String> idleBeat(IdleBeatRequest idleBeatRequest) {
        JobThread jobThread = OrthJobExecutor.loadJobThread(idleBeatRequest.getJobId());
    ...
    public Response<String> run(TriggerRequest triggerRequest) {
        ...
        JobThread jobThread = OrthJobExecutor.loadJobThread(triggerRequest.getJobId());
    ...
    private HandlerLoadResult loadBeanHandler(
        ...
        IJobHandler newJobHandler =
                OrthJobExecutor.loadJobHandler(triggerRequest.getExecutorHandler());
    ...
    public Response<String> kill(KillRequest killRequest) {
        JobThread jobThread = OrthJobExecutor.loadJobThread(killRequest.getJobId());

        if (jobThread != null) {
            OrthJobExecutor.removeJobThread(killRequest.getJobId(), "Admin scheduler kill request");
    ...
    public Response<LogResult> log(LogRequest logRequest) {
        ...
        LogResult logResult = OrthJobFileAppender.readLog(logFileName, logRequest.getFromLineNum());
